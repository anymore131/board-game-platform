...
<div class="body">
    <form action="/manage/grantUser" method="post">
        <table>
            <tr>
                <td>用户名：</td>
                <td>${grantUser.userName}</td>
            </tr>
            <tr>
                <td>姓名：</td>
                <td><input type="text" name="name" value="${grantUser.name}"></td>
            </tr>
            <tr>
                <td colspan="2">
                    年龄：<input type="number" name="age" max="100" min="0" value="${grantUser.age}">
                    性别：
                    <select id="gender" name="gender">
                        <c:if test="${grantUser.gender == 0}">
                            <option value="0" selected>女</option>
                            <option value="1">男</option>
                        </c:if>
                        <c:if test="${grantUser.gender == 1 || grantUser.gender == null}">
            ...
            <tr>
                <td>简介：</td>
                <td>
                    <textarea name="introduction">${grantUser.introduction}</textarea>
                </td>
            </tr>
            <tr>
                <td>位置：</td>
                <td><input type="text" name="province" value="${grantUser.province}"></td>
            </tr>
            <tr>
                <td></td>
                <td><input type="text" name="city" value="${grantUser.city}"></td>
            </tr>
            <tr>
                <td>手机号：</td>
                <td><input type="text" name="mobile" value="${grantUser.mobile}"></td>
            </tr>
            <tr>
                <td>邮箱：</td>
                <td><input type="text" name="email" value="${grantUser.email}"></td>
            </tr>
            <tr>
                <td>QQ：</td>
                <td><input type="text" name="QQ" value="${grantUser.QQ}"></td>
            </tr>
            <tr>
                <td>微信：</td>
                <td><input type="text" name="weixin" value="${grantUser.weixin}"></td>
            </tr>
            <tr>
                <td>用户权限：</td>
                <td>
                    <select id="type" name="type">
                        <c:if test="${grantUser.type == 0 || grantUser.type == null}">
                        <option value="0" selected>普通用户</option>
                        <option value="1">管理员</option>
                        </c:if>
                        <c:if test="${grantUser.type == 1}">
